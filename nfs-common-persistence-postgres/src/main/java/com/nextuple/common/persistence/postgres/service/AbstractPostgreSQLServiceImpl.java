...
public abstract class AbstractPostgreSQLServiceImpl<PK extends PostgresEntityKey, PV extends PostgresEntity,
    ...
    @Autowired
    GenericEntityMapper<PK, PV, EK, EV> mapper;
    ...
    @Override
    @Timed(value = "postgres.persistence", extraTags = {OPERATION_TAG_NAME, DB_OPERATION_READ})
    public EV get(EK key) {
        return repository.findById(mapper.entityKeyToDtoKey(key)).map(pv -> mapper.dtoToEntity(pv)).orElse(null);
    }

    @Override
    @Timed(value = "postgres.persistence", extraTags = {OPERATION_TAG_NAME, DB_OPERATION_READ_ALL})
    public Set<EV> getAll(Set<EK> keys) {
        Set<EV> values = new HashSet<>();
        repository.findAllById(keys.stream().map(ek -> mapper.entityKeyToDtoKey(ek)).collect(Collectors.toList()))
                .forEach( pv -> values.add(mapper.dtoToEntity(pv)));
        return values;
    }

    @Override
    @Timed(value = "postgres.persistence", extraTags = {OPERATION_TAG_NAME, DB_OPERATION_DELETE})
