package com.hbc.common.cache.service;

import static com.hbc.common.cache.constants.CacheConstants.*;

import com.hbc.common.cache.dto.CacheValue;
import com.hbc.common.cache.dto.key.CacheKey;
import com.hbc.common.cache.mapper.GenericCacheAndEntityMapper;
...
import com.hbc.common.util.ObjectUtil;
...
public abstract class AbstractCacheService<
  ...
  @Autowired GenericBaseCacheService<CK, CV> baseCacheService;

  @Autowired GenericPersistenceService<EK, ED> persistenceService;

  @Autowired GenericCacheAndEntityMapper<CK, CV, EK, ED> cacheAndEntityMapper;
  ...
  @Timed(
      value = "cache",
      extraTags = {OPERATION_TAG_NAME, CACHE_OPERATION_READ})
  public CV get(CK key) {
    Timer coreCacheReadTimer =
        meterRegistry.timer("cache.core", OPERATION_TAG_NAME, CACHE_OPERATION_READ);
    ...
    if (log().isInfoEnabled()) {
      log()
          .info(
              String.format(
                  "Time taken to fetch from cache is %d for key %s ",
    ...
    if (ObjectUtil.isNull(cv)) {
      ...
      Timer dbTimer =
          meterRegistry.timer("cache.persistence", OPERATION_TAG_NAME, DB_OPERATION_READ);
      cv = cacheAndEntityMapper.toCacheValue(persistenceService.get(entityKey));
      if (log().isInfoEnabled()) {
        log()
            .info(
                String.format(
                    "Time taken to fetch from db is %d for key %s ",
                    System.currentTimeMillis() - dbTimeStart, entityKey));
      }
  ...
  @Timed(
      value = "cache.core",
      extraTags = {OPERATION_TAG_NAME, CACHE_OPERATION_PUT})
  private CV put(CK key, CV value, int ttlInSeconds, boolean updateDb) {
    ...
    if (log().isInfoEnabled()) {
      log()
          .info(
              String.format(
                  "Time taken to put value into cache is %d for key %s ", end - start, key));
    }
    if (updateDb) {
      updateDb(key, value);
    }
  ...
  @Timed(
      value = "cache.persistence",
      extraTags = {OPERATION_TAG_NAME, DB_OPERATION_UPSERT})
  private void updateDb(CK key, CV value) {
    EK entityKey = cacheAndEntityMapper.toEntityKey(key);
    long start = System.currentTimeMillis();
    // TODO - perf- can we avoid returning
    persistenceService.update(
        cacheAndEntityMapper.toEntityKey(key), cacheAndEntityMapper.toEntity(key, value));
    long end = System.currentTimeMillis();
    if (log().isInfoEnabled()) {
      log()
          .info(
              String.format(
                  "Time taken to upsert value into db is %d for key %s ", end - start, entityKey));
    }
  }
  ...
  public void delete(CK key) {
    Timer coreCacheTimer =
        meterRegistry.timer(
            "cache.core", CACHE_KEY, key.toString(), OPERATION_TAG_NAME, CACHE_OPERATION_DELETE);
    long start = System.currentTimeMillis();
    ...
    if (log().isInfoEnabled()) {
      log()
          .info(
              String.format(
                  "Time taken to delete from the cache is %d for key %s ", end - start, key));
    }
  }
  ...
  public CV remove(CK key) {
    Timer coreCacheTimer =
        meterRegistry.timer(
            "cache.core", CACHE_KEY, key.toString(), OPERATION_TAG_NAME, CACHE_OPERATION_REMOVE);
    long start = System.currentTimeMillis();
    ...
    if (log().isInfoEnabled()) {
      log()
          .info(
              String.format(
                  "Time taken to remove from the cache is %d for key %s ", end - start, key));
    }
    return value;
  }
}
