...
public class TransitBufferReqJobRefService {
    ...
    public static final TransitBufferReqJobRefMapper INSTANCE = Mappers.getMapper(TransitBufferReqJobRefMapper.class);
    ...
    public List<TransitBufferReqJobRefResponse> getTransitBufferReqJobRefByExtReferenceId(String id)
        ...
        try
        {
            List<TransitBufferReqJobRefEntity> transitBufferReqJobRefEntityList = transitBufferReqJobRefRepository.findByExtReferenceId(id);

            if (!transitBufferReqJobRefEntityList.isEmpty()) {
                List<TransitBufferReqJobRefResponse> result = new ArrayList<>();
                transitBufferReqJobRefEntityList.forEach(entity -> result.add(INSTANCE.toTransitBufferReqJobRefDto(entity)));
                ...
                throw new TransitBufferReqJobRefDomainException(
                        "Unable to find transit buffer job references with this ID: " + id,
                        null,
                        id);
            }
        }
