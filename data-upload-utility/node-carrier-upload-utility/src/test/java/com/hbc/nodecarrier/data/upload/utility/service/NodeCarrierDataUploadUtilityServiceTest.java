package com.hbc.nodecarrier.data.upload.utility.service;
...
import com.hbc.nodecarrier.data.upload.utility.util.TestUtil;
...
class NodeCarrierDataUploadUtilityServiceTest {

  @InjectMocks NodeCarrierDataUploadUtilityService nodeCarrierDataUploadUtilityService;
  ...
  @BeforeEach
  public void setUp() {
    ReflectionTestUtils.setField(nodeCarrierDataUploadUtilityService, "basePath", "");
    ReflectionTestUtils.setField(nodeCarrierDataUploadUtilityService, "maxSizeInKiloBytes", 10240);
    ReflectionTestUtils.setField(nodeCarrierDataUploadUtilityService, "maxRows", 1000);
  }
  ...
  void uploadNodeCarrierDataSuccessTest() throws CommonServiceException, IOException {
    ...
    ResponseEntity<BaseResponse<String>> response =
        nodeCarrierDataUploadUtilityService.uploadNodeCarrierData(absolutePath);

    assertEquals(HttpStatus.OK, response.getStatusCode());
    assertEquals("Node Carrier Data successfully uploaded!", response.getBody().getMessage());
  }
  ...
  void uploadNodeCarrierInvalidHeadersExceptionTest() {
    ...
    assertEquals("Node Carrier data uploaded file has invalid headers.", exception.getMessage());
  ...
  void uploadNodeCarrierInvalidFileTypeExceptionTest() {
    ...
    assertEquals("Node Carrier data uploaded file is not a csv file.", exception.getMessage());
    assertEquals(HttpStatus.BAD_REQUEST, exception.getHttpStatus());
  }

  @Test
  void uploadNodeCarrierLargeFileSizeExceptionTest() {
    ReflectionTestUtils.setField(nodeCarrierDataUploadUtilityService, "maxSizeInKiloBytes", 1);
  ...
  void uploadNodeCarrierEmptyRecordsExceptionTest() {
    ...
    assertEquals("Node Carrier data uploaded file has no records.", exception.getMessage());
    assertEquals(HttpStatus.BAD_REQUEST, exception.getHttpStatus());
  }

  @Test
  void uploadNodeCarrierLargeRowSizeExceptionTest() {
    ReflectionTestUtils.setField(nodeCarrierDataUploadUtilityService, "maxRows", 30);
  ...
  void uploadNodeCarrierDataFailureTest() throws CommonServiceException, IOException {
    ...
    assertEquals(HttpStatus.BAD_REQUEST, response.getStatusCode());
    assertEquals("Node Carrier Data upload failed!", response.getBody().getMessage());
  }
