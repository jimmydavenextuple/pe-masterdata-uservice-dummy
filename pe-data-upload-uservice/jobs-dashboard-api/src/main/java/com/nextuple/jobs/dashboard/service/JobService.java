...
public class JobService {
  ...
  public JobResponse processJobJsonOffline(
    ...
    try {
      ...
      if (jobId.isPresent() && !ObjectUtils.isEmpty(jobId.orElse(null))) {
        var jobDto = jobsConsumerClient.getJob(orgId, jobId.get(), orgId).getPayload();
        ...
        if (!ObjectUtils.isEmpty(jobDto.getFileMetaDataId())) {
          ...
          BaseResponse<FileMetaDataResponse> fileMetaDataResponse =
              fileMetaDataClient.findFileMetadataById(jobDto.getFileMetaDataId(), orgId);
  ...
  public JobResponse processFileContents(
      JobDto jobDto, List<Object> uploadRequestList, InputStream inputStream)
      throws IOException, CsvException, JobDomainException, CommonServiceException {
    ...
    uploadRequestList.addAll(processFileContents.updateRequestObjectsList(jobDto, inputStream));
    log.debug(
    ...
    jobResponse = jobsConsumerClient.updateJob(jobDto, jobDto.getOrgId()).getPayload();

    var finalJobResponse = jobResponse;
    long totalKafkaPublishTime = getTotalKafkaPublishTime(uploadRequestList, finalJobResponse);
    log.debug(
  ...
  private long getTotalKafkaPublishTime(
      List<Object> uploadRequestList, JobResponse finalJobResponse) {
  ...
  private void publishToKafka(
    ...
    message =
        MessageBuilder.withPayload(recordDto)
            ...
            .build();
    message = authService.setAuthHeaders(message);
    try {
      ...
      kafkaTemplate
          ...
          .whenComplete(
              (sr, ex) -> {
                if (ex != null) {
                  log.error("JobService::publishToKafka():failed to publish record dto", ex);
                } else {
                  log.debug("JobService::publishToKafka():success to publish record dto", sr);
                }
              });
  ...
  public RecordInputDto parseRecordInputs(String data, int totalRecords) {
    var recordInput = JsonUtil.convertToObject(data, RecordInputDto.class);
    recordInput.put("retryCount", Integer.valueOf(totalRecords));
    return recordInput;
  }
}
