...
public class JobDomain {
  ...
  public JobEntity getAndUpdateJobStatusByStatus(JobStatusEnum oldStatus, JobStatusEnum newStatus)
      throws JobDomainException {
    try {
      var jobEntity = jobRepository.getJobStatusByStatus(oldStatus.name());
      ...
      logger.error(
          "Error while updating the job with for given orgId: {} and oldStatus: {}", oldStatus, e);
  ...
  @Transactional(propagation = Propagation.REQUIRES_NEW)
  public JobEntity getAndUpdateJobStatusByStatus(
      String jobId, String orgId, JobStatusEnum oldStatus, JobStatusEnum newStatus)
      throws JobDomainException {
    try {
      var jobEntity = jobRepository.findJobByJobIdAndOrgIdAndStatus(jobId, orgId, oldStatus);
  ...
  @Transactional(propagation = Propagation.REQUIRES_NEW)
  public JobEntity fetchJobRecordInTimeRange(String status, Date date) throws JobDomainException {
    try {
      return jobRepository.fetchJobRecordInTimeRange(status, date);
    } catch (Exception e) {
