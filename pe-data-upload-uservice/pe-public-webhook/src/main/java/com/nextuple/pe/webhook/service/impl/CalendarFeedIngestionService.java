...
public class CalendarFeedIngestionService
    implements FeedIngestionService<MasterDataIngestionDto<CalendarFeedDto>> {
  @Value("${master-data.calendar.topic-names:null}")
  private String calendarFeedTopic;

  @Value("${kafka-topic-flags.master-data.calendar.enabled:false}")
  ...
  @Override
  public void publish(
      String orgId, FeedRequest<MasterDataIngestionDto<CalendarFeedDto>> calendarFeed) {
    log.debug("Inside calendar publishing service with feed : {}", calendarFeed);
    calendarFeed
        .getData()
        .forEach(
            calendarDto -> {
              ...
              kafkaProducer.publishFeedToKafka(
                  batchRequest, calendarDto.getPayload().getCalendarId(), calendarFeedTopic);
            });
  }
  ...
  public boolean isPublishEnabled() {
    ...
    log.warn("Not publishing calendar feed messages since it is disabled.");
    return false;
  }
}
