...
public class TransitBufferFeedIngestionService
  ...
  @Override
  public void publish(
      String orgId, FeedRequest<MasterDataIngestionDto<TransitBufferFeedDto>> transitBufferFeed) {
    log.debug("Inside transit buffer publishing service with feed : {}", transitBufferFeed);
    transitBufferFeed
        .getData()
        .forEach(
            transitBufferDto -> {
              transitBufferDto.getPayload().setOrgId(orgId);
              BatchRequest<TransitBufferFeedDto> batchRequest = new BatchRequest<>();
              batchRequest.setAction(transitBufferDto.getAction());
              batchRequest.setProducedTimestamp(transitBufferDto.getProducedTimestamp());
              batchRequest.setPayload(transitBufferDto.getPayload());
              kafkaProducer.publishFeedToKafka(
                  batchRequest,
                  transitBufferDto.getPayload().getCarrierServiceId(),
                  transitBufferFeedTopic);
            });
  }
  ...
  public boolean isPublishEnabled() {
    ...
    log.warn("Not publishing transit buffer feed messages since it is disabled.");
    return false;
  }
}
