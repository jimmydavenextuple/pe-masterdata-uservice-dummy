...
class NodeFeedHandlingServiceTest {
  @Mock NodeBatchServiceImpl nodeBatchService;
  @Mock NodeFeedIngestionService nodeFeedIngestionService;
  @InjectMocks NodeFeedHandlingService nodeFeedHandlingService;
  ...
  void processNodeRecordsTest() {
    ...
    List<BatchRequest<NodeFeedDto>> nodeBatchFeed =
        Collections.singletonList(testUtil.getNodeFeedRequest(ActionEnum.CREATE));
    Mockito.when(objectMapper.convertValue(Mockito.eq(batchFeed), Mockito.any(TypeReference.class)))
        .thenReturn(nodeBatchFeed);

    Mockito.when(nodeBatchService.processRecordsWithoutRetry(any())).thenReturn(batchResponse);

    BatchResponse result = nodeFeedHandlingService.processRecords(batchFeed, TestUtil.ORG_ID);
    assertEquals(batchResponse, result);

    Mockito.verify(nodeBatchService).processRecordsWithoutRetry(any());
  }

  @Test
  void publishNodeRecordsTest() {
    FeedRequest<MasterDataIngestionDto<?>> batchRequest =
        testUtil.getNodeFeedIngestionRequest(ActionEnum.CREATE);

    Mockito.doNothing().when(nodeFeedIngestionService).publishFeedToKafka(any(), any());
    Mockito.when(
    ...
    Assertions.assertDoesNotThrow(
        () -> nodeFeedHandlingService.publishRecords(batchRequest, TestUtil.ORG_ID));

    Mockito.verify(nodeFeedIngestionService).publishFeedToKafka(any(), any());
  }
}
