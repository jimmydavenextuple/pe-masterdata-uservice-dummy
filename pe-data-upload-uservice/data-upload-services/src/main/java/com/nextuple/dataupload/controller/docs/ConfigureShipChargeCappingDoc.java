...
@Inherited
@Operation(
    summary = "Configure ship charge capping",
    description = "Configure ship charge capping for given organization ID.")
@ApiResponse(
    responseCode = "200",
    description = "A 200 status code indicates that ship charge capping configured successfully.",
    content =
        @Content(
            schema = @Schema(implementation = ConfigureShipChargeCappingResponse.class),
            examples = {
              @ExampleObject(
                  summary = "Attribute details fetched successfully",
                  name =
                      "A 200 status code indicates that ship charge capping configured successfully.",
                  value =
                      """
                                 {
                                     "success": true,
...
@ApiResponse(
    ...
    description =
        "A 400 error code indicates that there is an issue with the input."
            + "<ul>"
            + "<li><b>Error code: 16777032</b>: Error while configuring ship charge capping</li>"
            + "</ul>",
    content =
        @Content(
            ...
            examples = {
              @ExampleObject(
                  summary = "Error while configuring ship charge capping",
                  value =
                      """
                            {
                                "success": false,
                                ...
                                "message": "Error while configuring ship charge capping",
                                "payload": {
                                    "type": "ERROR",
                                    "code": 16777032
                                }
...
public @interface ConfigureShipChargeCappingDoc {}
