...
@Inherited
@Operation(
    summary = "Fetch target gross profit margin",
    description =
        "Fetches target gross profit margin for given attribute name and organization ID.")
@ApiResponse(
    responseCode = "200",
    description = "A 200 status code indicates that target profit margin fetched successfully.",
    content =
        @Content(
            schema = @Schema(implementation = TargetProfitMarginResponse.class),
            examples = {
              @ExampleObject(
                  summary = "Target profit margin fetched successfully",
                  name =
                      "A 200 status code indicates that target profit margin fetched successfully.",
                  value =
                      """
                                 {
                                     "success": true,
...
@ApiResponse(
    ...
    description =
        "A 400 error code indicates that there is an issue with the input."
            + "<ul>"
            + "<li><b>Error code: 16777029</b>: Error while fetching target profit margin records</li>"
            + "</ul>",
    content =
        @Content(
            ...
            examples = {
              @ExampleObject(
                  summary = "Error while fetching target profit margin records",
                  value =
                      """
                            {
                                "success": false,
                                ...
                                "message": "Error while fetching target profit margin records",
                                "payload": {
                                    "type": "ERROR",
                                    "code": 16777028
                                }
...
public @interface FetchTargetProfitMarginDoc {}
