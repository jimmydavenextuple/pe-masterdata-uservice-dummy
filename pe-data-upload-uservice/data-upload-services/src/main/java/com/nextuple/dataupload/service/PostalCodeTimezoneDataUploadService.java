...
public class PostalCodeTimezoneDataUploadService {
    ...
    try (Reader reader = Files.newBufferedReader(path);
      ...
      for (CSVRecord csvRecord : csvParser) {
        ...
        try {
          ...
          String zipCodePrefix = csvRecord.get(ZIP_CODE_PREFIX);
          ...
          switch (action) {
            case CREATE:
              {
                postalCodeResult =
                    isCreatePostalCodeTimezoneSuccessful(
            ...
            case UPDATE:
              {
                postalCodeResult =
                    isUpdatePostalCodeTimezoneSuccessful(
                        orgId, zipCodePrefix, country, state, city, latitude, longitude, timeZone);
                break;
              }
            ...
            case DELETE:
              {
                postalCodeResult = isDeletePostalCodeTimezoneSuccessful(orgId, zipCodePrefix);
                break;
              }
          ...
          if (isAllPassedForPostalCode) isAllPassedForPostalCode = false;
          log.error("Failed to store Postal Code Timezone CSV data for row number : {}", row);
        }

        if (isAllPassedForPostalCode) isAllPassedForPostalCode = postalCodeResult;
        if (isAllFailedForPostalCode) isAllFailedForPostalCode = !postalCodeResult;
      }
      return DataUploadUtil.storeToMap(isAllPassedForPostalCode, isAllFailedForPostalCode);
    }
  }

  private boolean isDeletePostalCodeTimezoneSuccessful(String orgId, String postalCodePrefix) {
  ...
  private boolean isUpdatePostalCodeTimezoneSuccessful(
      String orgId,
  ...
  private boolean isCreatePostalCodeTimezoneSuccessful(
      String orgId,
      String postalCodePrefix,
      String country,
      String state,
    ...
    var createPostalCodeTimezoneRequest =
        CreatePostalCodeTimezoneRequest.builder()
            .orgId(orgId)
            .zipCodePrefix(postalCodePrefix)
            .country(country)
            .state(state)
            .city(city)
            .latitude(latitude)
            ...
            .build();
    BaseResponse<PostalCodeTimezoneDto> baseResponse =
        postalCodeTimezoneFeign.createPostalCodeTimezone(createPostalCodeTimezoneRequest);
    log.debug(baseResponse.getMessage());
    return baseResponse.isSuccess();
  }
}
