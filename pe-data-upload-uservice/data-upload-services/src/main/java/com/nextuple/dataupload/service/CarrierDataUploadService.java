...
public class CarrierDataUploadService {
        ...
        try {
          ...
          switch (action) {
            case CREATE:
              {
                carrierResult =
                    isCreateCarrierServiceSuccessful(
                        orgId,
                        carrierId,
                        carrierServiceId,
                        carrierName,
            ...
            case UPDATE:
              {
                carrierResult =
                    isUpdateCarrierServiceSuccessful(
                        orgId,
                        carrierId,
                        carrierServiceId,
                        carrierName,
            ...
            case DELETE:
              {
                carrierResult =
                    isDeleteCarrierServiceSuccessful(orgId, carrierId, carrierServiceId);
                break;
              }
          ...
          if (isAllPassedForCarrier) isAllPassedForCarrier = false;
          log.error("Failed to store Carrier CSV data for row number : {}", row);
        }

        if (isAllPassedForCarrier) isAllPassedForCarrier = carrierResult;
        if (isAllFailedForCarrier) isAllFailedForCarrier = !carrierResult;
      }
      return DataUploadUtil.storeToMap(isAllPassedForCarrier, isAllFailedForCarrier);
    }
  }

  private boolean isDeleteCarrierServiceSuccessful(
      String orgId, String carrierId, String carrierServiceId) {
  ...
  private boolean isCreateCarrierServiceSuccessful(
      String orgId,
      String carrierId,
  ...
  private boolean isUpdateCarrierServiceSuccessful(
      String orgId,
      String carrierId,
      String carrierServiceId,
      String carrierName,
