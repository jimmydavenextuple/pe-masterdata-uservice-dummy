...
public class UploadBufferService {
    ...
    try (Reader reader = Files.newBufferedReader(path);
      ...
      for (CSVRecord csvRecord : csvParser) {
        ...
        try {
          ...
          var nodeServiceOptionBufferRequest = new NodeCarrierBufferRequest();
          nodeServiceOptionBufferRequest.setNodeId(nodeId);
          nodeServiceOptionBufferRequest.setOrgId(orgId);
          nodeServiceOptionBufferRequest.setServiceOption(serviceOption);
          nodeServiceOptionBufferRequest.setBufferHours(bufferHours);
          nodeServiceOptionBufferRequest.setBufferStartDate(bufferStartDateTime);
          nodeServiceOptionBufferRequest.setBufferEndDate(bufferEndDateTime);

          BaseResponse<NodeCarrierResponse> baseResponse;
          baseResponse = nodeCarrierFeign.createBuffer(nodeServiceOptionBufferRequest);
  ...
  private Map<String, Boolean> transitBufferCsvReader(Path path)
    ...
    try (var reader = new BufferedReader(new FileReader(path.toFile()))) {
      ...
      validateBufferHours(orgIdValue, carrierServiceIdValue, startTimeValue, endTimeValue);
      ...
      for (TransitBufferCreationRequest transitBufferRequest : transitBufferCreationRequests) {
        try {
          transitBufferResult = isUpdateTransitBufferSuccessful(transitBufferRequest);
        } catch (Exception e) {
  ...
  private void validateBufferHours(
      String orgIdValue, String carrierServiceIdValue, String startTimeValue, String endTimeValue)
  ...
  private boolean isUpdateTransitBufferSuccessful(
      TransitBufferCreationRequest transitBufferRequest) {
  ...
  private String deleteTransitBufferCsvReader(Path path)
    ...
    try (var csvWriter =
      ...
      distinctSourceGeozones.add(0, DESTINATION_SOURCE_GEOZONE_HEADER);
