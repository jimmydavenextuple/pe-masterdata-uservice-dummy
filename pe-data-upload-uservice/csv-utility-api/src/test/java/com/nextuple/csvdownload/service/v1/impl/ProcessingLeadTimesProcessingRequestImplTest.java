...
class ProcessingLeadTimesProcessingRequestImplTest {
  ...
  @Captor ArgumentCaptor<String[]> leadTimeCaptor;
  ...
  public void init() {
    ...
    ReflectionTestUtils.setField(
        processingLeadTimesProcessingRequest, "recordsPerPage", TestUtil.RECORDS_PER_PAGE);
    ReflectionTestUtils.setField(
        processingLeadTimesProcessingRequest, "maxNoOfPages", TestUtil.MAX_NO_OF_PAGES);
  }
  ...
  @Test
  @DisplayName("Verify header names and data in error logs")
  void addErrorLineTest() throws IOException {
    CSVWriter csvWriter = mock(CSVWriter.class);
    List<RecordStatusDto> recordStatusDtos = testUtil.getRecordStatusDtoForLeadTime();

    processingLeadTimesProcessingRequest.addErrorLine(csvWriter, recordStatusDtos);
    verify(csvWriter, times(1)).writeNext(leadTimeCaptor.capture());
    String[] leadTimeRecord = leadTimeCaptor.getValue();
    Assertions.assertEquals("UPDATE", leadTimeRecord[0]);
    Assertions.assertEquals("NODE1", leadTimeRecord[1]);
    Assertions.assertEquals("XYZINC", leadTimeRecord[2]);
    Assertions.assertEquals("SDND", leadTimeRecord[3]);
    Assertions.assertEquals("2", leadTimeRecord[4]);
  }
}
