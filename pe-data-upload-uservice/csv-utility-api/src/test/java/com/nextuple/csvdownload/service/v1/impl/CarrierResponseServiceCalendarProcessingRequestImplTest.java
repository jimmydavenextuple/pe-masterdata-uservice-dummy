...
class CarrierResponseServiceCalendarProcessingRequestImplTest {
  ...
  @BeforeEach
  public void init() {
    ReflectionTestUtils.setField(carrierServiceCalendarProcessingRequest, "bucketName", "bucket");
    ReflectionTestUtils.setField(carrierServiceCalendarProcessingRequest, "storageType", "s3");
    ReflectionTestUtils.setField(
        carrierServiceCalendarProcessingRequest, "recordsPerPage", TestUtil.RECORDS_PER_PAGE);
    ReflectionTestUtils.setField(
        carrierServiceCalendarProcessingRequest, "maxNoOfPages", TestUtil.MAX_NO_OF_PAGES);
  }
  ...
  void downloadErrorLogs() throws CommonServiceException, JobSubmissionException, IOException {
    JobDto jobDto =
        testUtil.createJob(TestUtil.JOB_ID, JobTypeEnum.UPLOAD_CARRIER_SERVICE_CALENDER, 20);
    RecordStatusDto recordStatusDto = testUtil1.getJobRecordsForCarrierServiceCalendar();
    ...
    PreSignedUrlResponse preSignedUrlResponse =
        carrierServiceCalendarProcessingRequest.downloadErrorLogs(
  ...
  @Test
  void getJobType() {
    JobTypeEnum jobTypeEnum = carrierServiceCalendarProcessingRequest.getJobType();
    Assertions.assertEquals(JobTypeEnum.UPLOAD_CARRIER_SERVICE_CALENDER, jobTypeEnum);
  }
}
