...
import static com.nextuple.dataupload.common.constants.CommonDataUploadErrorConstants.CARRIER_SERVICE_DATA_UPLOAD_INVALID_FILE_HEADERS;
import static com.nextuple.dataupload.common.constants.CommonDataUploadErrorConstants.CARRIER_SERVICE_DATA_UPLOAD_INVALID_FILE_TYPE;
...
@Service
public class CarrierServiceProcessingRequestImpl extends AbstractProcessingRequest {

  @Value("${download-page-size.node-carrier-service-options}")
  private Integer noOfRecordsPerPage;

  public CarrierServiceProcessingRequestImpl(
  ...
  public String getModuleType() {
    return ModuleEnum.CARRIER_SERVICE.getModuleValue();
  }

  @Override
  public String submitJob(String orgId, long fileMetadataId) throws JobSubmissionException {
    return submitJob(orgId, JobTypeEnum.UPLOAD_CARRIER_SERVICE, fileMetadataId).getJobId();
  }
  ...
  public void validate(GenericUploadRequest request, FileResponse fileResponse)
    ...
    DataUploadUtil.validateFileType(
        fileResponse.getContentType(), CARRIER_SERVICE_DATA_UPLOAD_INVALID_FILE_TYPE);
    ...
    DataUploadUtil.validateCSVHeaders(
        ...
        getModuleType(),
        CARRIER_SERVICE_DATA_UPLOAD_INVALID_FILE_HEADERS,
        csvReader);
  ...
  public void addErrorLine(CSVWriter writer, List<RecordStatusDto> recordStatusDtos)
      throws IOException {
    recordStatusDtos.forEach(dto -> constructCarrierServiceError(writer, dto));
    writer.flush();
  }

  private void constructCarrierServiceError(CSVWriter writer, RecordStatusDto recordStatusDto) {
    var gson = new Gson();
    var requestBody = gson.fromJson(recordStatusDto.getRequestBody(), CarrierServiceUpload.class);

    var req =
        new String[] {
          requestBody.getAction(),
          requestBody.getOrgId(),
          requestBody.getCarrierId(),
          requestBody.getCarrierName(),
          requestBody.getCarrierServiceId(),
          requestBody.getServiceName(),
          requestBody.getServiceOptions(),
