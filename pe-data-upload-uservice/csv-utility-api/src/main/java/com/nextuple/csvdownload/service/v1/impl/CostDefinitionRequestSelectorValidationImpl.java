...
@Slf4j
public class CostDefinitionRequestSelectorValidationImpl
    implements ICostDefinitionRequestValidation {
  ...
  public Optional<String> getCostItinerary(
      CostDefinitionRequest request, CostTypeValidationResponse response) {
    return response.getSelectorCfInfo().stream()
        .filter(
            selector ->
                selector.getSelectorCfValue().equals(request.getSelector().getSelectorCfValue()))
        .map(SelectorCfInfo::getCostItinerary)
        .findFirst();
  }
  ...
  private static boolean isCostFactorValid(
      String requestFactor,
      String selectorCfValue,
      List<SelectorCfInfo> selectorCfInfo,
      Function<SelectorCfInfo, CostFactorDescriptionDto> descriptionDtoFunction) {
    return selectorCfInfo.stream()
        ...
        .filter(info -> !ObjectUtils.isEmpty(descriptionDtoFunction.apply(info)))
        .noneMatch(
            info -> requestFactor.equals(descriptionDtoFunction.apply(info).getCostFactor()));
  }
