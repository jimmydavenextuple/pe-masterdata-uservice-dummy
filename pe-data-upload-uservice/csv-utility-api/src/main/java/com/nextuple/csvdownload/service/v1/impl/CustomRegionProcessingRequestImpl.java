...
import static com.nextuple.dataupload.common.constants.CommonDataUploadErrorConstants.CUSTOM_REGION_DATA_UPLOAD_INVALID_FILE_HEADERS;
import static com.nextuple.dataupload.common.constants.CommonDataUploadErrorConstants.CUSTOM_REGION_DATA_UPLOAD_INVALID_FILE_TYPE;
...
@Service
public class CustomRegionProcessingRequestImpl extends AbstractProcessingRequest {

  @Value("${download-page-size.custom-region}")
  private Integer noOfRecordsPerPage;

  public CustomRegionProcessingRequestImpl(
  ...
  public String getModuleType() {
    return ModuleEnum.CUSTOM_REGION.getModuleValue();
  }

  @Override
  public String submitJob(String orgId, long fileMetadataId) throws JobSubmissionException {
    return submitJob(orgId, JobTypeEnum.UPLOAD_CUSTOM_REGION, fileMetadataId).getJobId();
  }
  ...
  public void validate(GenericUploadRequest request, FileResponse fileResponse)
    ...
    DataUploadUtil.validateFileType(
        fileResponse.getContentType(), CUSTOM_REGION_DATA_UPLOAD_INVALID_FILE_TYPE);
    ...
    DataUploadUtil.validateCSVHeaders(
        ...
        getModuleType(),
        CUSTOM_REGION_DATA_UPLOAD_INVALID_FILE_HEADERS,
        csvReader);
  ...
  public String tempFilePrefix() {
    return "download-log-custom-region";
  }

  @Override
  public void addErrorLine(CSVWriter writer, List<RecordStatusDto> recordStatusDtos)
      throws IOException {
    recordStatusDtos.forEach(dto -> constructCustomRegionError(writer, dto));
    writer.flush();
  }

  void constructCustomRegionError(CSVWriter writer, RecordStatusDto recordStatusDto) {
    var gson = new Gson();
    var requestBody = gson.fromJson(recordStatusDto.getRequestBody(), CustomRegionUpload.class);
    String geoZones =
        !requestBody.getCodes().isEmpty() ? String.join(":", requestBody.getCodes()) : "";
    var req =
        new String[] {
          requestBody.getAction(),
          requestBody.getOrgId(),
          requestBody.getId(),
          requestBody.getCustomRegionName(),
          requestBody.getCustomRegionDescription(),
          geoZones,
