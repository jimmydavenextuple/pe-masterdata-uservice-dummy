...
public interface NodeGroupFeign {

  @PostMapping("/node-group")
  BaseResponse<NodeGroupResponse> createNodeGroup(
      @Valid @RequestBody CreateNodeGroupRequest nodeGroupRequest);

  @GetMapping("/node-group/orgId/{orgId}/id/{id}")
  BaseResponse<NodeGroupResponse> getNodeGroupByIdandOrgId(
      @NotNull @PathVariable Long id, @NotBlank @PathVariable String orgId);

  @PutMapping("/node-group/orgId/{orgId}/id/{id}")
  BaseResponse<PromiseSourcingRuleDto> updateNodeGroup(
