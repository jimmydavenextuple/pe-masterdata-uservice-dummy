...
public interface SourcingRulesConfigurationFeign {

  @PostMapping("/sourcing-rules-configuration")
  BaseResponse<SourcingRuleDetails> configureSourcingRule(
      @Valid @RequestBody RulesConfigurationRequest rulesConfigurationRequest);

  @GetMapping("/sourcing-rules-configuration/orgId/{orgId}/id/{id}")
  BaseResponse<SourcingRuleDetails> getSourcingRuleDetailsByIdandOrgId(
      @NotNull @PathVariable Long id, @NotBlank @PathVariable String orgId);

  @DeleteMapping("/sourcing-rules-configuration")
  BaseResponse<SourcingRuleDetails> deleteSourcingRule(
      @NotBlank @RequestParam String orgId, @NotNull @RequestParam Long sourcingRuleId);

  @PostMapping("/sourcing-rules-configuration/fetch-rules")
  BaseResponse<FetchSourcingRulesResponse> fetchSourcingRules(
