...
public class SourcingConstraintFeignClientServiceImpl
  ...
  GenericMapper<
          ...
          String,
          BaseResponse<SourcingConstraintsResponse>>
      sourcingConstraintMapper;

  @Override
  public SourcingConstraintCacheValue get(SourcingConstraintCacheKey key) {
    try {
      return sourcingConstraintMapper.responseToCacheValue(
          sourcingConstraintFeign.fetchSourcingConstraintsList(key.getOrgId(), key.getGroupId()));
    } catch (RuntimeException ex) {
      return SourcingConstraintCacheValue.builder().build();
    }
  }
}
