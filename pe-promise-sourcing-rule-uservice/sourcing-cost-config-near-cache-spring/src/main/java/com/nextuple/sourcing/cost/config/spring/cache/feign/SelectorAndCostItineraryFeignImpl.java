...
public interface SelectorAndCostItineraryFeignImpl
    extends GenericFeignService<
        String, BaseResponse<List<SelectorAndCostItineraryMappingResponse>>> {

  @GetMapping("/cost-config/selector-itinerary-mapping/{orgId}")
  BaseResponse<List<SelectorAndCostItineraryMappingResponse>> get(@PathVariable String orgId);

  @GetMapping("/cost-config/selector-itinerary-mapping/{orgId}/costType/{costType}")
  BaseResponse<List<SelectorAndCostItineraryMappingResponse>>
      getSelectorAndCostItineraryMappingByOrgIdSelectorCfAndCostType(
          ...
          @NotBlank @PathVariable String costType,
          @RequestParam(required = false) String selectorCf);
}
