...
public class CostFactorFeignServiceImpl
  ...
  public CostFactorCacheValue get(CostFactorCacheKey key) {
    try {
      return costFactorMapper.responseToCacheValue(
          costFactorFeign.getCostFactorByOrgIdAndCostFactor(key.getOrgId(), key.getCostFactor()));
    } catch (RuntimeException ex) {
      return CostFactorCacheValue.builder().build();
    }
  }
}
