...
class TransitBufferReqJobRefControllerTest {
  ...
  void createTransitBufferReqJobRefTest()
    ...
    ResponseEntity<BaseResponse<TransitBufferReqJobRefResponse>> responseEntity =
        transitBufferReqJobRefController.createTransitBufferReqJobRef(
            transitBufferReqJobRefRequest);

    Assertions.assertEquals(HttpStatus.OK, responseEntity.getStatusCode());
    Assertions.assertEquals(
        testUtil.getTransitBufferReqJobRefResponse(), responseEntity.getBody().getPayload());
    verify(transitBufferReqJobRefService, times(1)).createTransitBufferReqJobRef(any());
  }

  @Test
  void createTransitBufferReqJobRefTestError()
      throws TransitBufferReqJobRefDomainException, CommonServiceException {
    TransitBufferReqJobRefRequest transitBufferReqJobRefRequest =
        testUtil.getTransBufferReqJobRefRequest();
    when(transitBufferReqJobRefService.createTransitBufferReqJobRef(any()))
  ...
  void getTransitBufferReqJobRefByExtRefId()
    ...
    TransitBufferReqJobRefResponse transitBufferReqJobRefResponse =
        testUtil.getTransitBufferReqJobRefResponse();
    ...
    Assertions.assertEquals(
        testUtil.getTransitBufferReqJobRefResponse(),
        responseEntity.getBody().getPayload().stream().findFirst().get());
    verify(transitBufferReqJobRefService, times(1))
        .getTransitBufferReqJobRefByExtReferenceId(any());
  }
  ...
  @Test
  void getTransitBufferReqJobRefByExtRefIdError()
      throws TransitBufferReqJobRefDomainException, CommonServiceException {
