...
@Inherited
@Operation(
    summary = "Delete Zone Details",
    description =
        "Deletes zone details when they are no longer required. This API deletes the zone information by passing the organization ID, source geo zone, destination geo zone, and carrier service ID attributes in the path parameters for the required zone.")
@ApiResponse(
    responseCode = "200",
    description = "A 200 success code indicates that the zone details are deleted successfully.",
    content =
        @Content(
            schema = @Schema(implementation = ZoneResponse.class),
            examples = {
              @ExampleObject(
                  summary = "When zone details are deleted successfully.",
                  name =
                      "A 200 success code indicates that the zone details are deleted successfully.",
                  value =
                      """
                                                {
                                                  "success": true,
                                                  ...
                                                  "message": "Zone data removed successfully",
                                                  "payload": {
                                                    "orgId": "NEXTUPLE_GR",
                                                    "sourceGeoZone": "M1T",
...
public @interface DeleteZoneDetailsDoc {}
