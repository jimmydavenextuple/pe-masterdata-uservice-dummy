...
class CurrentThreadContextTest {
  ...
  @DisplayName("Test for request context")
  @Test
  void getRequestContextTest() {
    CurrentThreadContext.cleanRequestContext();
    RequestContext actual = CurrentThreadContext.getRequestContext();
    assertNotNull(actual);
    assertNull(actual.get("test", null));

    CurrentThreadContext.setRequestContext(
        RequestContext.builder()
            .customRequestFields(Map.of("request-test", "request-test"))
            .build());
    assertEquals("request-test", CurrentThreadContext.getRequestContext().get("request-test"));
  }
}
