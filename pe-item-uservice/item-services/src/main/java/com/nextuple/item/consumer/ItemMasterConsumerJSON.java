...
@KafkaListener(
    topics = "${spring.kafka.consumer-item.topics.item_master.name}",
...
public class ItemMasterConsumerJSON {
  ...
  public void onItemMasterEventConsumption(
    ...
    } catch (ConstraintViolationException cve) { // NOSONAR
      logger.error(
          "Error while validating the item feed message: {}, {}",
          cve.getMessage(),
          itemMasterEvent);
      throw cve;
    } catch (Exception e) { // NOSONAR
      logger.error(
          "Error while consuming the item feed message: {}, {}", e.getMessage(), itemMasterEvent);
      throw e;
    }
  }
}
