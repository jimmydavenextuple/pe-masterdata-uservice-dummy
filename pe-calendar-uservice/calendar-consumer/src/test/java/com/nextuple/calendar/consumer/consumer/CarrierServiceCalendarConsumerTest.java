...
class CarrierServiceCalendarConsumerTest {

  @InjectMocks private CarrierServiceCalendarConsumer carrierServiceCalendarConsumer;
  @Mock private CarrierServiceCalendarBatchServiceImpl carrierServiceCalendarBatchService;
  @InjectMocks private TestUtil testUtil;

  @Test
  @DisplayName("When the carrier service calendar feed gets consumed successfully.")
  void consumeMasterDataFeedTest() {
    List<BatchRequest<CarrierServiceCalendarFeedDto>> carrierServiceCalendarFeedRequests =
    ...
    when(carrierServiceCalendarBatchService.processRecordsWithRetry(any()))
        .thenReturn(batchResponse);
    carrierServiceCalendarConsumer.consumeCarrierServiceCalendarFeed(
        carrierServiceCalendarFeedRequests, null);

    verify(carrierServiceCalendarBatchService, times(1)).processRecordsWithRetry(any());
  }
}
