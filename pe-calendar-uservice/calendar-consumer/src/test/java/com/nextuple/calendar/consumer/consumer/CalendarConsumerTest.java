...
class CalendarConsumerTest {

  @InjectMocks private CalendarConsumer calendarConsumer;
  @Mock private CalendarBatchServiceImpl calendarBatchService;
  @InjectMocks private TestUtil testUtil;

  @Test
  @DisplayName("When the calendar feed gets consumed successfully.")
  void consumeMasterDataFeedTest() {
    List<BatchRequest<CalendarFeedDto>> calendarFeedRequests =
    ...
    when(calendarBatchService.processRecordsWithRetry(any())).thenReturn(batchResponse);
    calendarConsumer.consumeCalendarFeed(calendarFeedRequests, null);

    verify(calendarBatchService, times(1)).processRecordsWithRetry(any());
  }
}
