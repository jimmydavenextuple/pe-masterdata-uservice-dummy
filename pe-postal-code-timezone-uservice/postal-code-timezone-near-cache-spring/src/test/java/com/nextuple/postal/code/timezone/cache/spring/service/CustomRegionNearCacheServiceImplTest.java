...
class CustomRegionNearCacheServiceImplTest {

  @InjectMocks private CustomRegionNearCacheServiceImpl customRegionNearCacheService;
  ...
  @Mock
  private GenericFeignCacheService<CustomRegionCacheKey, CustomRegionCacheValue> feignCacheService;
  ...
  void getTestForValidParameters() {
    ...
    // First Invocation
    CacheValue cacheValue1 = customRegionNearCacheService.get(cacheKey);
  ...
  void selfRegister() {
    ...
    customRegionNearCacheService.selfRegister();
  ...
  void getEntityName() {
    assertEquals(
        NearCacheConstants.SOURCING_CUSTOM_REGION_ENTITY_NAME,
        customRegionNearCacheService.getEntityName());
  }

  @Test
  void deleteTest() {
    CustomRegionCacheKey cacheKey = testUtil.getCustomRegionCacheKey();
    customRegionNearCacheService.delete(cacheKey);
    CacheValue cacheValue = customRegionNearCacheService.get(cacheKey);
  ...
  void deleteAllTest() {
    CustomRegionCacheKey cacheKey = testUtil.getCustomRegionCacheKey();
    customRegionNearCacheService.deleteAll();
    CacheValue cacheValue = customRegionNearCacheService.get(cacheKey);
    assertNull(cacheValue);
  }
}
