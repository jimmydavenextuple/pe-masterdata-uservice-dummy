...
public class ScheduledMethod {
  ...
  private final Logger logger = LoggerFactory.getLogger(ScheduledMethod.class);
  ...
  public void processJobOffline() throws JobDomainException {
    ...
    if (Objects.isNull(jobDto)) {
      jobDto = getJobInProcessingStateForADay(JobStatusEnum.PROCESSED.name(), timeRangeInHours);
    }
    ...
    try {
      ...
      logger.error("Error while processing job: {}", jobDto.getJobId(), e);
      jobDto.setStatus(JobStatusEnum.FAILED);
  ...
  private JobDto getJobInProcessingStateForADay(String status, int noOfHoursInPast)
