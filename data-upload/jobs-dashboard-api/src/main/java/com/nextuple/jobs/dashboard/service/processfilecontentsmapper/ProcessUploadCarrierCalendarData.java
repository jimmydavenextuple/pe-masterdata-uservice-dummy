...
public class ProcessUploadCarrierCalendarData implements ProcessFileContents {
  ...
  private List<CarrierCalendarUpload> createUploadMarketRegionsJobRequest(InputStream inputStream)
      throws IOException {
    ...
    var csvFormat =
        CSVFormat.DEFAULT.withHeader(
            DataUploadUtilityExpectedHeaders.getCSVExpectedHeaders(
                    ModuleEnum.CARRIER_SERVICE_CALENDAR.getModuleValue())
    ...
    iterator.next();
    List<CarrierCalendarUpload> carrierCalendarUploadList = new ArrayList<>();

    while (iterator.hasNext()) {
      var csvRecord = iterator.next();
      var postalCodeTimezoneUpload =
          CarrierCalendarUpload.builder()
              ...
              .shippingStage(csvRecord.get(SHIPPING_STAGE))
              .effectiveDate(csvRecord.get(EFFECTIVE_DATE))
              .description(csvRecord.get(DESCRIPTION))
              .build();
      carrierCalendarUploadList.add(postalCodeTimezoneUpload);
    }

    return carrierCalendarUploadList;
  }
}
