...
public class NodeCarrierCalendarDataUploadService {
  ...
  public ResponseEntity<BaseResponse<String>> uploadNodeCarrierCalendarData(String fileUri)
    ...
    List<String> expectedHeaders =
        DataUploadUtilityExpectedHeaders.getCSVExpectedHeaders("node-carrier-calendar");
    ...
    if (!FILE_TYPE.equals(Files.probeContentType(path))) {
      ...
      throw new CommonServiceException(
          NODE_CARRIER_CALENDAR_DATA_UPLOAD_INVALID_FILE_TYPE,
    ...
    if (sizeOfFileInKiloBytes > maxSizeInKiloBytes) {
      ...
      errorMap.put(
          ...
          FieldError.builder()
              ...
              .build());
      throw new CommonServiceException(
          NODE_CARRIER_CALENDAR_DATA_UPLOAD_LARGE_FILE_SIZE,
          HttpStatus.BAD_REQUEST,
          0x2774,
          errorMap);
    }
    ...
    if (numberOfRowsInFile < 2) {
      ...
      errorMap.put(FILE_URI, FieldError.builder().rejectedValue(fileUri).build());
      throw new CommonServiceException(
          NODE_CARRIER_CALENDAR_DATA_UPLOAD_FILE_EMPTY_RECORDS,
          HttpStatus.BAD_REQUEST,
          0x2775,
          errorMap);
    }
    ...
    if (numberOfRowsInFile > maxRows) {
      ...
      errorMap.put(
          ...
          FieldError.builder()
              ...
              .build());
      throw new CommonServiceException(
          NODE_CARRIER_CALENDAR_DATA_UPLOAD_LARGE_ROW_SIZE,
          HttpStatus.BAD_REQUEST,
          0x2776,
          errorMap);
    }
    ...
    if (resultMap.get("isAllPassed").equals(true)) {
      return ResponseEntity.ok(
          BaseResponse.builder()
              .message("Node Carrier Calendar Data successfully uploaded!")
              .build());
    }
    if (resultMap.get("isAllFailed").equals(true)) {
      return ResponseEntity.status(HttpStatus.BAD_REQUEST)
          .body(
              BaseResponse.builder().message("Node Carrier Calendar Data upload failed!").build());
    }
    return ResponseEntity.status(HttpStatus.MULTI_STATUS)
        .body(
            BaseResponse.builder()
                .message("Node Carrier Calendar Data partially uploaded with some rows failed!")
                .build());
  }
